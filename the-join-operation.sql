...
SELECT teamname, COUNT(player) AS total_number_of_goals
FROM eteam 
JOIN goal ON eteam.id = goal.teamid
GROUP BY teamname;

-- 10 - Show the stadium and the number of goals scored in each stadium:
SELECT stadium, COUNT(player) AS number_of_goals
FROM game 
join goal ON game.id = goal.matchid
GROUP BY stadium; 

-- 11 - For every match involving 'POL', show the matchid, date and the number of goals scored:
SELECT game.id, game.mdate, COUNT(player)
FROM game
JOIN goal ON game.id = goal.matchid
...
SELECT goal.matchid, game.mdate, COUNT(player) AS number_of_goals
FROM game
JOIN goal ON game.id = goal.matchid
WHERE teamid = 'GER'
